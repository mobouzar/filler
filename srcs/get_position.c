...
/*   Updated: 2019/11/07 17:24:03 by mobouzar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		is_placeable(t_filler *filler, int x, int y)
{
	...
	while (++i < filler->p_x)
	{
		...
		while (++j < filler->p_y)
		{
			if ((ft_tolower(filler->map[x + i][y + j]) == filler->me[0])
			&& filler->piece[i][j] == '*')
				count++;
			else if (ft_tolower(filler->map[x + i][y + j]) == filler->enemy[0]
			&& filler->piece[i][j] == '*')
				return (0);
		}
	}
